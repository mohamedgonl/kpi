/**
 * 6 Work Group Metadata Configuration
...
/**
 * Get all items across all groups as a flat list for dropdowns
 */
export function getAllItems() {
  const items = [];
  getWorkGroups().forEach(group => {
...
export function getGroupById(id) {
  return getWorkGroups().find(g => g.id === id);
}
...
export function getItemById(itemId) {
  for (const group of getWorkGroups()) {
    for (const sub of group.subGroups) {
      const item = sub.items.find(i => i.id === itemId);
      if (item) return { ...item, groupId: group.id, groupName: group.name };
    }
  }
  return null;
}
